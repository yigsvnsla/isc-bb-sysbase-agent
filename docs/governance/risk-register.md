...
| R04 | Consumo ilimitado (costo/DoS) | M | M | Rate limit Redis (60/min user, 20/min IP) + presupuestos diarios; `X-Forwarded-For` solo se confía desde `app.security.trusted-proxies` (evita bypass del límite por IP vía header spoofeado) | Mitigado (Fase 3) |
...
| R08 | Pérdida de auditoría | B | A | ai_audit en PG; métrica ai_audit_write_failures_total; alerta; WORM (F12a): export JSONL append-only con hash chain HMAC-SHA256 (clave en `app.audit.worm.hmac-secret`, no solo SHA-256 plano — evita que alguien con acceso de escritura al filesystem recalcule la cadena) + purge solo de exportados | Mitigado (Fases 2+12) |
