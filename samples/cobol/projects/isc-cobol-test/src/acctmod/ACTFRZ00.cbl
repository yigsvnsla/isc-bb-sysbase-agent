       *================================================================*
       * ACTFRZ00 - CONGELAR / DESCONGELAR CUENTA                     *
       * PROPOSITO: CAMBIAR ESTATUS ENTRE ACTIVO Y CONGELADO          *
       * EQUIPO: CONTABLE - 2005                                      *
       ...
       *
        01  WS-VARIABLES.
            ...
            05  WS-PROGRAMA-ID             PIC X(08) VALUE 'ACTFRZ00'.
        ...
        01  SCR-FREEZE.
            05  SCR-CABECERA.
                ...
                10  LINE 02  COL 01  PIC X(80)
                    VALUE ' CONGELAR / DESCONGELAR CUENTA'.
       ...
       *
                10  LINE 08  COL 05  PIC X(20) VALUE 'ACCION A REALIZAR:'.
                10  LINE 08  COL 28  PIC X(15) FROM WS-ACCION.
       *
                10  LINE 10  COL 05  PIC X(15) VALUE 'MOTIVO:'.
                10  LINE 10  COL 22  PIC X(40)
                    USING WS-MOTIVO AUTO.
       *
            05  SCR-PIE.
                ...
                10  LINE 24  COL 02  PIC X(78)
                    VALUE 'PF3=PROCESAR  PF11=AYUDA  PF12=RETORNAR'.
       *
       *================================================================*
        LINKAGE SECTION.
       *
       ...
       *
                WHEN WS-CRT-PF3
                    PERFORM 5000-PROCESAR
                    GO TO FRZ-LOOP
       *
                WHEN WS-CRT-PF11
                    CALL 'COMHELP' USING 'ACTFRZ00'
                    GO TO FRZ-LOOP
       *
                WHEN WS-CRT-PF12
                    MOVE 00 TO LS-RETCODE
       ...
       *
                WHEN WS-CRT-CLEAR
                    MOVE SPACES TO WS-ACTNBR WS-MOTIVO
                    MOVE SPACES TO WS-MENSAJE WS-MENSAJE-ERROR
                    GO TO FRZ-LOOP
       *
                WHEN OTHER
                    MOVE 'USE ENTER=CONS PF3=PROC PF12=SALIR'
                      TO WS-MENSAJE-ERROR
                    GO TO FRZ-LOOP
            END-EVALUATE.
       *
       ...
       *
        1000-INICIALIZAR.
            ...
            MOVE 'INGRESE CUENTA, ENTER=CONS, PF3=PROCESAR'
              TO WS-MENSAJE.
       ...
       *
            IF FL-ACCOUNT-STATUS = '00'
                IF ACT-STATUS = 'F'
                    MOVE 'DESCONGELAR CUENTA' TO WS-ACCION
                ELSE
                    IF ACT-STATUS = 'A'
                        MOVE 'CONGELAR CUENTA' TO WS-ACCION
                    ELSE
                        IF ACT-STATUS = 'C'
                            MOVE 'CTA CERRADA - NO APLICA'
                              TO WS-ACCION
                        ELSE
                            MOVE 'CONGELAR CUENTA' TO WS-ACCION
                        END-IF
                    END-IF
                END-IF
                MOVE 'CUENTA ENCONTRADA' TO WS-MENSAJE
            ELSE
       ...
       *
            IF WS-MOTIVO = SPACES
                MOVE 'INGRESE MOTIVO DE LA OPERACION'
                  TO WS-MENSAJE-ERROR
                GO TO 5000-EXIT
            END-IF.
       *
            MOVE SPACES TO WS-CONFIRMA.
            DISPLAY 'CONFIRMA OPERACION? (S/N): '
       ...
       *
            IF FL-ACCOUNT-STATUS = '00'
                ...
                IF ACT-STATUS = 'F'
                    MOVE 'CUENTA CONGELADA' TO WS-MENSAJE
                ELSE
                    MOVE 'CUENTA DESCONGELADA' TO WS-MENSAJE
                END-IF
            ELSE
       ...
       *
        ...
        6000-ESCRIBIR.
                ...
                MOVE 'ACTFRZ00' TO AUD-PROGRAMA
                MOVE 'BL' TO AUD-EVENTO
                MOVE 'CT' TO AUD-ENTITY-TYPE
                MOVE ACT-NBR TO AUD-ENTITY-KEY
                STRING WS-MOTIVO INTO AUD-OBSERVACIONES
                MOVE 'O' TO AUD-RESULTADO
                WRITE AUDITLOG-RECORD
                CLOSE AUDITLOG-FILE
            END-IF.
       *
        END PROGRAM ACTFRZ00.
